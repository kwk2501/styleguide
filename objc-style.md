# The official RecordFarm Objective-C style guide.
...
A three letter prefix should always be used for class names and constants, however may be omitted for Core Data entity names. For any official recordfarm.com books, starter kits, or tutorials, the prefix ‘RF’ should be used.
...
```objc
static NSTimeInterval const RFRecordViewControllerNavigationFadeAnimationDuration = 0.3;
```
...
```objc
static NSString * const RFAboutViewControllerCompanyName = @“recordfarm.com";

static CGFloat const RFImageThumbnailHeight = 50.0;
```

**Not Preferred:**

```objc
#define CompanyName @“recordfarm.com"
...
```objc
typedef NS_ENUM(NSInteger, RFLeftMenuTopItemType) {
  RFLeftMenuTopItemMain,
  RFLeftMenuTopItemShows,
  RFLeftMenuTopItemSchedule
};
```
...
```objc
typedef NS_ENUM(NSInteger, RFGlobalConstants) {
  RFPinSizeMin = 1,
  RFPinSizeMax = 5,
  RFPinCountMin = 100,
  RFPinCountMax = 500,
...
```objc
RFLeftMenuTopItemType menuType = RFLeftMenuTopItemMain;

switch (menuType) {
  case RFLeftMenuTopItemMain:
    // ...
    break;
  case RFLeftMenuTopItemShows:
    // ...
    break;
  case RFLeftMenuTopItemSchedule:
    // ...
    break;
}
```
...
Private properties should be declared in class extensions (anonymous categories) in the implementation file of a class. Named categories (such as `RFPrivate` or `private`) should never be used unless extending another class.   The Anonymous category can be shared/exposed for testing using the <headerfile>+Private.h file naming convention.
...
```objc
@interface Airplane
+ (instancetype)airplaneWithType:(RFAirplaneType)type;
@end
```
...
*[NSHipster - Documentation](http://nshipster.com/documentation/)
*[NSHipster - #pragma](http://nshipster.com/pragma/)
